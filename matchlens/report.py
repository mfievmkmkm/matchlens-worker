...
def build_report(job_dir:Path,frames:list[dict],tracker_id:int,fps:float):
    metrics=calculate_player_metrics(frames,tracker_id,fps)
...
<div class="card"><h2>Цифры</h2><p>≈ Дистанция в кадре: <b>{metrics['distance_m']} м</b></p><p>≈ Максимальная скорость: <b>{metrics['max_speed_kmh']} км/ч</b></p><p>✅ Наблюдаемые касания: <b>{metrics['touches_observed']}</b></p></div>
